...
class ResourceSchedule(json: JSONObject) {
    ...
    val events = (0 until json.getJSONArray("ActivityData").length()).map {
        ResourceEvent(json.getJSONArray("ActivityData").getJSONObject(it))
    }

    val weeks = (0 until json.getJSONArray("WeekData").length()).map {
